...
BEGIN
  ...
  IF TG_OP = 'DELETE' THEN
    ...
    FROM milestones m
    WHERE OLD.milestone_id = m.id AND m.phase_id IS NOT NULL;
  ELSE
    -- For inserts/updates, use NEW record
    SELECT ARRAY_AGG(DISTINCT m.phase_id) INTO affected_phases
    FROM milestones m
    WHERE NEW.milestone_id = m.id AND m.phase_id IS NOT NULL;
  END IF;
...
CREATE TRIGGER update_phase_dates_on_task_change
  ...
  FOR EACH ROW 
  WHEN (
    (TG_OP = 'INSERT' AND NEW.milestone_id IS NOT NULL) OR
    (TG_OP = 'UPDATE' AND (OLD.milestone_id IS DISTINCT FROM NEW.milestone_id OR OLD.start_date IS DISTINCT FROM NEW.start_date OR OLD.end_date IS DISTINCT FROM NEW.end_date)) OR
    (TG_OP = 'DELETE' AND OLD.milestone_id IS NOT NULL)
  )
...
CREATE TRIGGER update_phase_dates_on_milestone_change
  ...
  FOR EACH ROW 
  WHEN (
    (TG_OP = 'INSERT' AND NEW.phase_id IS NOT NULL) OR
    (TG_OP = 'UPDATE' AND OLD.phase_id IS DISTINCT FROM NEW.phase_id) OR
    (TG_OP = 'DELETE' AND OLD.phase_id IS NOT NULL)
  )
